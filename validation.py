...
def generate_validation_data(env, env_params, num_val=1000):
    ...
    val_rng, val_subkey_theta, val_subkey_tdot, val_subkey_act = jax.random.split(val_rng, 4)
    val_theta = jax.random.uniform(val_subkey_theta, (num_val,), minval=-jnp.pi, maxval=jnp.pi)
    ...
    val_true_delta_obs = ground_truth.true_delta_obs(env, env_params, val_theta, val_tdot, val_act[:, 0])
    val_true_reward = ground_truth.true_reward(env, env_params, val_theta, val_tdot, val_act[:, 0])

    return val_x, val_true_delta_obs, val_true_reward


def evaluate_validation(model, env, env_params, val_x, val_true_delta_obs, val_true_reward, dataset, pointer, rng, j):
    ...
    rand_theta = jax.random.uniform(subkey_theta, (num_rand,), minval=-jnp.pi, maxval=jnp.pi)
    ...
    true_delta_obs_rand = ground_truth.true_delta_obs(env, env_params, rand_theta, rand_tdot, rand_act[:, 0])
    true_reward_rand = ground_truth.true_reward(env, env_params, rand_theta, rand_tdot, rand_act[:, 0])

    # Plot true vs predicted
    plotting.plot_true_vs_predicted(
        true_delta_obs, pred_delta_obs,
        true_delta_obs_rand, pred_delta_obs_rand,
        true_reward, pred_reward,
        true_reward_rand, pred_reward_rand,
        j
    )
